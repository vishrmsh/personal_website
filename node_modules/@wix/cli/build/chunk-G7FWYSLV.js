...
async function assertBuildOutputExists(projectFolder) {
  ...
  const targets = [
    ...
    ...buildManifest.serverDir ? [buildManifest.serverDir] : []
  ];
...
function useLoadBuildFiles() {
  ...
  return useAsyncCallback(async () => {
    ...
    const client = (await loadFiles(buildManifest.clientDir)).filter(
      ({ path: path2 }) => {
        if (path2 === buildManifest.appManifestPath) {
          return false;
        }
        if (buildManifest.serverDir && path2.startsWith(buildManifest.serverDir)) {
          return false;
        }
        return true;
      }
    );
    const server = (buildManifest.serverDir ? await loadFiles(buildManifest.serverDir) : []).filter(({ path: path2 }) => {
      if (path2 === buildManifest.appManifestPath) {
        return false;
      }
      return true;
...
function useDeployFiles() {
  ...
  return useAsyncCallback(async (_, files) => {
    ...
    const completeAppDeployment = await backendAsAServiceClient.completeAppDeployment(
      {
        ...appDeployment,
        files: backendFiles
      },
    ...
    return {
      ...
      deploymentBaseUrl: completeAppDeployment.deploymentBaseUrl
    };
  });
}
...
var Preview = ({ environment, baseUrl, label, onPreviewCreated }) => {
  ...
  const createPreviewTagAction = useAsyncCallback(
    async (_, serverUrl, deploymentId) => {
      ...
      const gitCommitHash = await isInsideGitRepo(model.projectFolder) ? await getGitLatestCommitHash(model.projectFolder) : void 0;
  ...
  const { status: createPreviewStatus } = useAsync(async () => {
    ...
    const { deploymentUrl: previewUrl, deploymentId } = await uploadApplicationAction.execute(baseUrl);
    const previewId = await createPreviewTagAction.execute(
      previewUrl,
      deploymentId
    );
  ...
  return /* @__PURE__ */ import_react2.default.createElement(import_react2.default.Fragment, null, /* @__PURE__ */ import_react2.default.createElement(
    ...
    Error: () => null,
    Loading: () => null,
    Success: ({ result: { previewId, previewUrl } }) => /* @__PURE__ */ import_react2.default.createElement(Box_default, { rowGap: 1, marginY: 1, flexDirection: "column" }, /* @__PURE__ */ import_react2.default.createElement(Text, null, "Open the preview on:"), /* @__PURE__ */ import_react2.default.createElement(Box_default, { paddingLeft: 2, flexDirection: "column" }, /* @__PURE__ */ import_react2.default.createElement(PlatformLink, { url: previewUrl, label: "Site" }), /* @__PURE__ */ import_react2.default.createElement(
      PlatformLink,
      {
        url: `https://manage.wix.com/dashboard/${model.config.siteId}?apps-override=${previewId}`,
        label: "Dashboard"
      }
    )))
  }));
...
//# sourceMappingURL=chunk-G7FWYSLV.js.map